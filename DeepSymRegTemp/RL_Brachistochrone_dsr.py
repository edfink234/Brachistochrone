...
#        y = best_vals[1]
    ...
    with tf.compat.v1.Session() as sess:
        ...
        try:
            while True:
                while (expr := str(loop()))=="0":
                    sess.run(tf.compat.v1.global_variables_initializer())
                    print("Trying again")
                x = sp.symbols('x')
                expr = sp.sympify(str(expr))
                model_selection = lambdify(x, expr)
                y_hat = model_selection(X)
                if isinstance(y_hat, float):
                    y_hat = np.full_like(X, fill_value = y_hat)
                loss = np.sum((y_hat.flatten()-y.flatten())**2)
                if loss < best_loss:
                    print("New best loss:", best_loss:=loss)
            ...
            if best_loss < 0.06550575070015369:
                ...
                elif y_points.shape != x_points.shape:
                    y_points = np.full_like(x_points, fill_value=y_points).shape
                plt.title(f"Number of Sampled Points = {len(X)}")
                plt.plot(x_points, model_selection(x_points),
                         label=rf"f(x) = ${sp.latex(best_expr)}$")
                plt.legend()
                # Save the figure and show your friends! :)
                print("Saved!")
                plt.savefig("RL_Brachistochrone_dsr.png", dpi=5 * 96)
